...
**API 숙지:**
[Mojo GPU Memory Operations](https://max.modular.com/mojo/std/gpu/memory/)
...
- **[`copy_dram_to_sram_async()`](https://max.modular.com/mojo/layout/layout_tensor/copy_dram_to_sram_async/)**:
  전용 복사 엔진을 사용하여 백그라운드 DRAM→SRAM 전송 시작
- **[`async_copy_wait_all()`](https://max.modular.com/mojo/std/gpu/memory/memory/async_copy_wait_all/)**:
...
- **타일 생성**: `input.tile[CONV_TILE_SIZE](block_idx.x)`는
  ...
  Mojo의
  [`tile` 메서드](https://max.modular.com/mojo/layout/tile_tensor/TileTensor/#tile)는
