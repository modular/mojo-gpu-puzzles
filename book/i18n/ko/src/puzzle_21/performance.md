...
메모리 접근 패턴을 이해하는 것은 GPU 성능 최적화의 핵심입니다. 이 섹션에서는
임베딩 조회와 같은 메모리 바운드 연산에서 병합 메모리 접근 패턴이 왜 비병합
패턴보다 뛰어난 성능을 보이는지 설명합니다.

## 메모리 병합 기초

**메모리 병합**은 워프 내 연속된 스레드가 연속된 메모리 주소에 접근할 때
발생합니다. GPU는 이러한 개별 메모리 요청을 더 적은 수의 대용량 메모리
트랜잭션으로 결합하여 대역폭 활용도를 크게 향상시킵니다.
...
- **왜 병합되는가**: `Thread 0: output[0,0,0]`, `Thread 1: output[0,0,1]` →
  연속된 주소
...
핵심 통찰: 특히 임베딩과 같은 메모리 바운드 연산에서는 연산 복잡도보다
**메모리 접근 패턴**이 GPU 성능을 결정하는 경우가 많습니다.
