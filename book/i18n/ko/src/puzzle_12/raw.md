...
벡터 `a`와 벡터 `b`의 내적을 계산하여 `output`(단일 값)에 저장하는 커널을 구현하세요.
...
- 여러 값을 하나로 합치는 병렬 리덕션(parallel reduction) 구현하기
- 공유 메모리에 중간 결과 저장하기
...
공유 메모리를 활용한 병렬 리덕션 알고리즘으로 내적을 계산하는 솔루션입니다. 단계별로 살펴보겠습니다:
...
1. **메모리 접근 패턴**:
   - 각 스레드가 전역 메모리에서 정확히 두 값을 로드 (`a[i]`, `b[i]`)
   - 중간 결과에 공유 메모리 사용
   - 최종 결과는 전역 메모리에 1회 기록
...
이 구현은 순차 실행의 \\(O(n)\\)에 비해 \\(O(\log n)\\) 시간 복잡도를 달성하며, 병렬 리덕션 알고리즘의 위력을 보여줍니다.
