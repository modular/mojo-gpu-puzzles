...
벡터 `a`와 벡터 `b`의 내적을 계산하여 `output`(단일 값)에 저장하는 커널을 구현하세요. 내적은 크기가 같은 두 벡터에서 대응하는 원소끼리 곱한 뒤, 그 결과를 모두 더해 하나의 숫자(스칼라)를 구하는 연산입니다.
...
수동 메모리 관리와 동기화로 리덕션을 밑바닥부터 구현하는 방법을 알아봅니다.

### [📐 TileTensor 버전](./layout_tensor.md)

TileTensor를 활용해 리덕션과 공유 메모리 관리를 더 간결하게 구현합니다.

💡 **참고**: TileTensor로 메모리 접근 패턴이 얼마나 깔끔해지는지 확인해 보세요. 효율은 그대로입니다.
