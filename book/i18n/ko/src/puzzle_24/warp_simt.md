...
### 공유 메모리 bank conflict

**Bank conflict란?**

GPU 공유 메모리가 동시 접근이 가능한 32개의 독립적인 **banks**로 나뉘어 있다고 가정합니다. **Bank conflict**는 Warp 내 여러 스레드가 같은 뱅크의 서로 다른 주소에 동시에 접근하려 할 때 발생합니다. 이 경우 하드웨어가 접근을 **직렬화**해야 하므로, 단일 사이클이어야 할 연산이 여러 사이클로 늘어납니다.

**핵심 개념:**

- **충돌 없음**: 각 스레드가 서로 다른 뱅크에 접근 → 모든 접근이 동시에 발생 (1 사이클)
- **Bank conflict**: 여러 스레드가 같은 뱅크에 접근 → 접근이 순차적으로 발생 (N개 스레드에 N 사이클)
