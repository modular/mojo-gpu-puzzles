...
행렬 `a`의 각 위치에 10을 더해 `output`에 저장하는 커널을 구현해 보세요.
...
3. 가드 내부: 행 우선 방식으로 10을 더하는 방법을 생각해 보세요!
...
원시 메모리로 2D 블록 기반 처리를 구현할 때의 핵심 개념을 보여주는 솔루션입니다:
...
2. **메모리 레이아웃**
   - 행 우선 선형 메모리: `index = row * size + col`
   - 5×5 행렬 예시:
