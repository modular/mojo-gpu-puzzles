...
- **오류 분석**: 크래시 메시지와 stack trace (_역주: 오류 발생 시점까지의 함수 호출 경로_) 해석하기
...
3. 이 null 포인터가 GPU kernel로 전달됩니다.
4. Kernel이 `a[i]`에 접근하려 할 때 null을 역참조 → `CUDA_ERROR_ILLEGAL_ADDRESS`
...
**💡 핵심 통찰**: 이런 유형의 null 포인터 버그는 GPU 프로그래밍에서 매우 흔합니다. 여기서 배운 체계적인 CUDA-GDB 조사 방법은 다른 많은 GPU 메모리 문제, 경쟁 상태, kernel 크래시를 디버깅할 때도 그대로 적용됩니다.
